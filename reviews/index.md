...
**Current ship (living):** `src/dns-cli` **1.25.0** — login doorbell `/usr/local/bin/dns-review-hooks` (heal rewrites old `dns-cli`, `dns-cli-hook`, and `login-review-hook`); TTY main menu invalid pick reprints the same numbered layer (main + sudoers submenu); inbound DNS `submit` / `approve` / `reject` / `interactive` **Implemented**; login-hook `interactive` shows the waiting body as YAML and keeps the latest duplicate inbound per dest; dest fence catalog `requirement-approval-fencing-condition`; dest Fence `requirement-incorrect-json-format`; hyphenated login in DNS request names; Type 0 **test-purpose** `fence-test`; TTY main menu daily DNS work + family **sudoers** submenu; independent login-hook REQ (`requirement-login-interactive-hook`). Historical rows below keep the verdict they had on that date.
