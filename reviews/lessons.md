...
| L-STOR-01 | Shared world-writable storage; about names cache only | `util_resolve_storage` + persistency `${HOME}/.local/dns-cli`; TP-CLI-12 · **TP-CLI-17** | open watch |
...
| L-SUDOER-PATH-01 | Sudoer JSON emit copies `$GLOBAL_BIN`; tests redirect that to `.ci-homes/…/gbin`; dest applied it to `/etc/sudoers.d/dns-cli-dns-adm`. Login `sudo -n /usr/local/bin/dns-cli interactive` then fails. Law identity is `/usr/local/bin/dns-cli`, not the install-isolation knob | Pin emit+verify+hook to `/usr/local/bin/dns-cli`; `CF_TEST_LPU` does not detect live dest inbound; **TP-SUDOER-JSON-22** · **TP-SUDOER-JSON-24**. Do not rewrite `/etc/sudoers.d` from dns-cli. Incident **20260821-001** | open watch |
| L-F5-DIR-01 | F4 `~/dns-request` `ls` Permission denied: setup `chmod 3773` inbound then `lpu_chown_tree`s only `/var/dns-cli`. Children stay `root:root`. `3773` other cannot list. F4 symlink does not add rights. **003** “MUST NOT chown inbound JSON” is not “MUST NOT chown the inbound directory” | `chown dns-adm:dns-adm` each F5 **directory** (no `-R` on inbound files). Keep `3773`. Do not treat `ls -l` of the symlink as listable inbound. Incident **20260823-001** | open watch |
...
| L-BN-HYPHEN-01 | Dest basename parse used `cut -d- -f2/-f3` so POSIX logins with hyphens failed dest format | Date from the left; action + `n` from the right; REQ-M6; **TP-CF-REQ-18** | open watch |
