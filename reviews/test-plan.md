...
### TP-CF-APR (login-hook plant / heal)

| TP-ID | Intent | Suite | Primary requirement(s) | Status |
|-------|--------|-------|------------------------|--------|
| TP-CF-APR-01 | interactive approver heals `.bashrc` hook | `tests/test_cf_approver.sh` | requirement-login-interactive-hook | **have** |
| TP-CF-APR-02 | missing `.profile` created and sources `.bashrc` | test_cf_approver | requirement-login-interactive-hook | **have** |
| TP-CF-APR-03 | existing `.profile` not overwritten | test_cf_approver | requirement-login-interactive-hook | **have** |
| TP-CF-APR-07 | rc heal aligns owner to corresponding user | test_cf_approver | requirement-login-interactive-hook HOOK-M4 · term shell-rc-file-ownership | **have** |
...
| TP-CF-REQ-11 | login-hook `interactive` takes inbound ownership at the beginning | test_cf_request | requirement-dns-actor-table ACT-M4 · requirement-login-interactive-hook | **have** |
