...
**Updated:** 2026-09-16 (default-interaction 1.7.0 family **self-management** is main **8**)
...
| requirement-project-folder | Project layout (`src/dns-cli`), install bins; LPU home + vault pointer; persistency pointer | architecture | Active (2.4.0) | `requirement-project-folder.md` | 2026-08-30 |
...
| requirement-shell-cli-interface | Shell CLI interface; **dual mention** + **CI-M1a samples** on topic-owners; Type 0 **test-purpose** `test-json-format` / `fence-test`; **`menu`/`main`**; family `DNS Features` / `sudoers` not dispatched; `interactive` also login-hook REQ | shell | Active (3.12.0) | `requirement-shell-cli-interface.md` | 2026-09-16 |
| requirement-shell-cli-default-interaction | Case 3 numbered main menu: family **DNS Features** **1** + family **sudoers** **7** + family **self-management** **8**; Exit **9**; finished command returns to the top list; invalid pick reprints the same layer; header **`${APP_NAME}`**(*`${VERSION}`*) - `${SHORT_DESC}`; row explain SGR 3;37 | shell | Active (1.7.0) Implemented | `requirement-shell-cli-default-interaction.md` | 2026-09-16 |
...
| requirement-shell-cli-zero-arguments | Empty argv Type N: TTY numbered menu; off-TTY help (no DNS/vault mutate) | shell | Active | `requirement-shell-cli-zero-arguments.md` | 2026-09-03 |
...
| requirement-dns-actor-table | Actor table; YAML review display; keep-latest duplicate inbound at login-hook `interactive`; dest interactive dest-writes `submit_by`; hook plant points at independent REQ | architecture | Active (1.13.0) Implemented | `requirement-dns-actor-table.md` | 2026-09-08 |
...
| requirement-cloudflare-dns-request | Four inbound JSON types; YAML review display; keep-latest duplicate inbound; dest-written `submit_by`; hyphenated subject in basename | domain | Active (1.8.0) Implemented | `requirement-cloudflare-dns-request.md` | 2026-09-06 |
