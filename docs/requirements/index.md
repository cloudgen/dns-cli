...
**Updated:** 2026-09-13 (login doorbell `/usr/local/bin/dns-review-hooks`; default-interaction 1.4.0 invalid menu pick reprints the same numbered layer)
...
| requirement-shell-cli-interface | Shell CLI interface; **dual mention** + **CI-M1a samples** on topic-owners; Type 0 **test-purpose** `test-json-format` / `fence-test`; **`menu`/`main`**; family `sudoers` not dispatched; `interactive` also login-hook REQ | shell | Active (3.11.0) | `requirement-shell-cli-interface.md` | 2026-09-08 |
| requirement-shell-cli-default-interaction | Case 3 numbered main menu: daily DNS work + family **sudoers** submenu; invalid pick reprints the same layer; header **`${APP_NAME}`**(*`${VERSION}`*) - `${SHORT_DESC}`; row explain SGR 3;37 | shell | Active (1.4.0) Implemented | `requirement-shell-cli-default-interaction.md` | 2026-09-13 |
...
| requirement-shell-modular-function-design | Single-file modular prefixes; `util_*` example ownership; sudo/prompt/temp pointers; `app_default_*` sudoers submenu | shell | Active (2.5.0) | `requirement-shell-modular-function-design.md` | 2026-09-03 |
