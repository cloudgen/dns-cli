...
| requirement-least-privilege-user | LPU `dns-adm` F1–F7; dest Fence points at `requirement-incorrect-json-format`; L-M14 test-mode inbound; L-M15 login-hook-symlink is `dns-review-hooks` | architecture | Active (1.15.0) Implemented | `requirement-least-privilege-user.md` | 2026-09-13 |
| requirement-three-layer-privilege-model | Type 0/1/2 map + role table + Tables A/B/C; dest Fence points at independent REQ; P-M14 test-mode inbound; ELEV-CF-02 `dns-review-hooks` | architecture | Active (1.16.0) Type 1 + two-kind submitter + Type 2 switch Implemented | `requirement-three-layer-privilege-model.md` | 2026-09-13 |
...
| requirement-shell-local-self-management | Local **`self-install`** (alias `install`) / uninstall / where-is-me; copy `$0` (no download); dest **0700** local / **0755** global | shell | Active (1.8.0) | `requirement-shell-local-self-management.md` | 2026-09-17 |
...
| requirement-shell-cli-storage | Cache folder + persistency folder (not vault); storage `util_*` examples | shell | Active (1.4.0) | `requirement-shell-cli-storage.md` | 2026-08-30 |
