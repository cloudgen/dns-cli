...
**Status**: Active (Version 1.1.0) Implemented  
**Area**: shell  
...
dns-cli **claims** a default function: a **numbered main menu** of live operational commands. `requirement-shell-cli-zero-arguments` exists and the product is **not** online-installable. Empty argv **MUST** stay help (**case 3**). The menu **MUST** attach to routed-verb **`menu`** (alias **`main`**).

### 1.1 Human-facing

**In one sentence:** At a real terminal, `dns-cli menu` shows **dns-cli**(*version*) - short description, then numbered jobs whose explain text is light gray italics; in a script the same command prints help and does not wait.

| Box | Meaning | Example |
|-----|---------|---------|
| You / this login | Open the list, pick a number | `dns-cli menu` then `6` |
| The other role | CI / pipe | `dns-cli menu` = help |
| Not this file | Empty argv, or install/version on the list | `dns-cli` still help |
...
| `dns-cli menu` | Command | Numbered list |

| You do… | What it means | What you type |
|---------|---------------|---------------|
| Open the board at a prompt | Header names the copy and the short description, then numbered jobs | `dns-cli menu` |
| Leave | Exit | `99` |
...
Claimed **yes**. Specialized zero-argument requirement **exists**. Product is **non-online-installable**. This is **case 3**: empty argv **MUST** stay Type N help. The menu **MUST** be verb **`menu`**. **`main` MUST** alias the same handler.
...
Labels **MUST** be `command: what it does`. On TTY, the describe text after `: ` (and the Exit label) **MUST** be light gray (SGR 90) and italic (SGR 3) via `app_default_print_row`. Off-TTY: the same words, no CSI.
...
| Product | `dns-cli` |
| Claimed | yes |
| Case | **3** — empty argv stays Type N help; menu is verb `menu`/`main` |
| Exit | 99 |
| Header | `util_app_ident` + ` - ${SHORT_DESC}` → **`${APP_NAME}`**(*`${VERSION}`*) - `${SHORT_DESC}` |
| Row explain | TTY SGR 90 + SGR 3 via `app_default_print_row` |
| Handler | `app_default` |
...
- **Principle 16 – Interactive**: No hang off-TTY; empty argv stays help.  
...
3. Hang CI on `dns-cli menu` off-TTY, or steal empty argv from Type N help.  
...
| `docs/requirements/requirement-shell-cli-zero-arguments.md` | Empty argv = help |
...
|----------------|-------|--------|
| **TP-CLI-07** | `tests/test_cli.sh` | have (empty argv stays help) |
| **TP-CLI-18** | `tests/test_cli.sh` | have (header `${APP_NAME}(${VERSION}) - ${SHORT_DESC}`; TTY bold name / italic version / gray-italic explain; off-TTY menu = help) |
